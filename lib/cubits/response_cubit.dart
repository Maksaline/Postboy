...
class ResponseCubit extends Cubit<ResponseState> {
  ...
  void loadResponse(
    ...
    dynamic newValue;
    if(automationMap != null && automationMap.isNotEmpty && count != null && count > 0 && bodyMap != null && bodyMap.isNotEmpty) {
      for(int i=0; i<count; i++) {
        ...
        try{
          ...
          newRes['${i+1}-StatusCode'] = '${response.statusCode} ${response.statusMessage}' ?? 0;
          ...
          for (var map in expectedMap.entries) {
            ...
            if (response.data is List) {
              ...
              if (response.data != null && response.data[map.key] != null) {
                if (map.value == "<<<Anything>>>" ||
                    response.data[map.key].toString() == map.value.toString()) {
                  keysChecked[i] =
                  keysChecked[i] != null ? keysChecked[i]! + 1 : 1;
                }
              }
